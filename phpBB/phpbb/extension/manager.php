...
class manager
{
	...
	*/
	public function is_available($name)
	{
		...
		try
		{
			return $md_manager->validate('all');
		}
	...
	*/
	public function version_check(\phpbb\extension\metadata_manager $md_manager, $force_update = false, $force_cache = false, $stability = null)
	{
		...
		{
			// Preserve the public behaviour expected by current ACP/CLI callers.
		...
		$version_helper->set_file_location(
			$version_check['host'],
			$version_check['directory'],
			$version_check['filename'],
			isset($version_check['ssl']) ? $version_check['ssl'] : false
		);
